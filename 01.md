...
- controller의 경우 어노테이션을 통해 자동생성되는 기능과  개별 API 등을 테스트 해야된다. Mock은 우리가 목업제품이라고 부르고 있는 그 목업의 의미로, 대체 또는 가짜의 의미를 가지고 있다. 또한 대체 객체를 하나하나 일일이 만드는 것보다는 Mockito라는 라이브러를 사용하여 controller의 test코드를 만들었다.
